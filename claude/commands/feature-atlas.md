...
backend), keeping `.claude/feature-atlas/` as the up-to-date source of truth for what exists and
...
2. **Detect conventions** — determine project type (Laravel / OpenCart / other) and read the
   project's root `CLAUDE.md` if present. This gets passed to every subagent below so findings
   respect the project's actual conventions instead of generic defaults.

3. **Ensure the output folder exists** — `.claude/feature-atlas/`. If `SUMMARY.md` doesn't exist,
   this is a first run.

4. **Run discovery** — spawn the `feature-atlas-mapper` agent with the project root, detected
...
7. **Fan out per-subsystem work** — for each new or stale subsystem id, run in sequence:
   ...
   using the DETAILS.md the scout just wrote). Multiple subsystems' scout→auditor pipelines can run
   concurrently — batch them (roughly 4-6 concurrent `Agent` calls at a time) rather than launching
   dozens at once.
...
10. **Synthesize** — spawn `feature-atlas-synthesizer` to independently verify every finding,
...
12. **Offer the project CLAUDE.md pointer** — if the project's root `CLAUDE.md` doesn't already
    reference `.claude/feature-atlas/`, propose adding a short section: that it's the source of
...
**Notes:**
- This command never edits project source files — only files under `.claude/feature-atlas/` and,
