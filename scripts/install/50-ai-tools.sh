...
for f in CLAUDE.md RTK.md settings.json statusline-command.sh; do
  backup_and_link ~/.claude/"$f" "$SCRIPTPATH/ai/$f"
done
...
if command_exists opencode; then
  ...
  backup_and_link ~/.config/opencode/agent "$SCRIPTPATH/ai/agents-opencode"
  backup_and_link ~/.config/opencode/command "$SCRIPTPATH/ai/commands"
  backup_and_link ~/.config/opencode/skill "$SCRIPTPATH/ai/skills"
...
if command_exists codex; then
  ...
  for d in "$SCRIPTPATH"/ai/codex-skills/*/; do
    name=$(basename "$d")
    backup_and_link ~/.codex/skills/"$name" "$SCRIPTPATH/ai/codex-skills/$name"
  done
